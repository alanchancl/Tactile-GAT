...
#         BatchNorm_size = embed_dim*2 if graph_name == '' else embed_dim
#         self.bn_outlayer_in = nn.BatchNorm1d(BatchNorm_size)
...
#         self.classlinear = nn.Linear(in_features = node_num, out_features = 20)
#         self.init_params()
...
#         x = data.clone().detach() ##[256, 27, 15]
#         edge_index_sets = self.edge_index_sets ##[2, 702]

#         device = data.device ##cuda

#         batch_num, node_num, all_feature = x.shape ##batch_num = 256, node_num = 27, all_feature = 15
...
#             if (len(edge_index_sets) ==1 and self.graph_name == 'g') or (len(edge_index_sets) ==2 and i == 0):
...
#                 all_embeddings = all_embeddings.repeat(batch_num, 1)##[27*128(3456), 64]
...
#                 gcn_out = self.gnn_layers[i](x, batch_gated_edge_index, node_num=node_num*batch_num, embedding=None)
#             elif (len(edge_index_sets) ==1 and self.graph_name == 'g+') or (len(edge_index_sets) ==2 and i == 1):
#                 batch_gated_edge_index = get_batch_edge_index(edge_index, batch_num, node_num).to(device)
#                 gcn_out = self.gnn_layers[i](x, batch_gated_edge_index, node_num=node_num*batch_num, embedding=None)
